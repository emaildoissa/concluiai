...
export function MultistoreDashboard() {
  ...
  async function requestAdjustment(ev: EvidenceRow, unitName: string, taskTitle: string) {
    ...
    try {
      ...
      setMsg({
        text: `Reexecução solicitada! Tarefa reaberta (${r.status})${r.notified ? ` · Notificação enviada via WhatsApp para ${r.notified}` : ''
          }`,
        type: 'success',
      });
      await loadData();
      ...
      <div className="ops-kpi-grid">
        ...
        <div className="ops-kpi-card" style={{ ['--kpi-glow' as string]: '#6366f1' }}>
          ...
          <div className="ops-kpi-body">
            ...
            <span
              className={`ops-kpi-benchmark ${avgScore && avgScore >= 85
                  ? 'benchmark-good'
                    ...
                    <div key={t.id} className={`incident-card ${cardCls}`}>
                      <div className="incident-header-row">
                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.65rem', flexWrap: 'wrap' }}>
                          <span
                            className={`badge ${STATUS_CLASS[t.status] || (t.isLate ? 'badge-late' : 'badge-pending')
                              }`}
                          >
              ...
              <div className="ops-evidences-grid">
                {filteredEvidences.map((ev) => {
                  ...
                  return (
                    <div
                      key={ev.id}
                      className={`ops-evidence-card ${isRejected ? 'card-rejected' : isApproved ? 'card-approved' : ''
                        }`}
                    >
                      <div
                      ...
                      >
                        ...
                        <div className="ops-evidence-badges">
                          <span
                            className={`badge ${isApproved ? 'badge-completed' : isRejected ? 'badge-critical' : 'badge-pending'
                              }`}
                          >
              ...
              <div className="ops-units-list">
                {rankedAndFilteredUnits.map((u, index) => {
                  ...
                  return (
                    <div
                      key={u.unit_id}
                      className={`ops-unit-card ${isCritical ? 'risk-high' : isWarning ? 'risk-mid' : 'risk-low'
                        }`}
                    >
                      <div className="ops-unit-top">
                        ...
                        <div className="ops-unit-score-badge">
                          ...
                          <span
                            className={`badge ${isCritical ? 'badge-critical' : isWarning ? 'badge-pending' : 'badge-completed'
                              }`}
            ...
            <div className="ops-lightbox-grid">
              ...
              <div className="ops-lightbox-details">
                <div className="ops-lightbox-header">
                  ...
                  <span
                    className={`badge ${selectedEvidence.reviewStatus === 'approved'
                        ? 'badge-completed'
